...
public class PersonDomainDataAssembler {
    ...
    /**
     * Assembler to person dto.
     * @param personJPA - person jpa.
     * @return The person dto.
     */
    public JpaToDomainPersonDTO toDomain(PersonJPA personJPA)
            throws InvalidEmailException, InvalidPersonNameException, InvalidVATException, InvalidDateException {
        ...
        return new JpaToDomainPersonDTO(personName, birthDate, vat, address, telephoneNumbersList, mainEmail, familyID, ledgerID, otherEmailsList,
                accountIDList);
    }
    ...
    public PersonJPA toData(Person person) {
        ...
        for (Email email : otherEmails) {
            personJPA.addEmail(new EmailJPA(email.toString(), personJPA));
        }
